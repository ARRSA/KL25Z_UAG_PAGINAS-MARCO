...
#define n_clock ((GPIOC_PDIR & 0x0F) == 0x00)
#define n_data	((GPIOC_PDIR & 0x0F) == 0x08)
...
void paginas(void)
{
	switch(select_page)
	{
		...
		case 0x0C:
			...
			break;
		case 0x03:
			select_page = page[129];
			break;
		case 0x0B:
			select_page = page[161];
			break;
		case 0x83:
			select_page = page[193];
			break;
		case 0x0A:
			select_page = page[225];
			break;
	}
}
...
void change_page(void)
{
		if((select_page = page[32]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[64]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[96]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[128]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[160]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[192]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[224]))
		{
			select_page++;
			sendCode(nIns,0x80);
		}
		if((select_page = page[256]))
		{
			select_page = page[0];
			sendCode(nIns,0x80);
		}
